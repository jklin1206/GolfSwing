...
def build_report_context(scorer, player="Player", rank=None, total=None):
    """
...
def render_template_report(ctx):
    ...
    L = []
    rank = ""
    if ctx["rank"] and ctx["of"]:
        rank = f" - Rank {ctx['rank']} of {ctx['of']}"
    L.append(f"# {ctx['player']} - Swing Score: {ctx['score']:.1f}{rank}")
    ...
    if ctx["fixes"]:
        L.append("## Try this next")
        L.append("Run these drills a few reps each before your next swing:")
        for f in ctx["fixes"]:
            L.append(f"- {f['title']}: {f['feel']}")
        L.append("")
...
# ---------------------------------------------------------------------------
POLISH_SYSTEM = (
    "You are a warm, encouraging golf coach writing a one-page take-home report for someone "
    "who just did a swing at a fun AI expo booth. Many readers have never golfed. "
    "You are given (a) structured facts about their swing and (b) a draft report. "
    "Rewrite the draft into friendly, personal, plain-English prose that a non-golfer enjoys "
    "reading and might show a friend.\n\n"
    "HARD RULES:\n"
    ...
    "- Keep every measured number (e.g. '166 degrees') - that specificity is the wow factor.\n"
    "- Lead with encouragement, keep it to roughly one page, no jargon without a plain gloss.\n"
    "- Keep the closing disclaimer. Output GitHub-flavoured Markdown."
)
